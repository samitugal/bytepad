  ...
  if (result.toolCalls && result.toolCalls.length > 0) {
    ...
    if (needsFollowUp && toolResults.length > 0) {
      ...
      const followUpMessages = [
        ...messages,
        { role: 'assistant', content: `[Tool çağrıları yapıldı - sonuçlar aşağıda]\n\n${toolResultsSummary}` },
        { role: 'user', content: `Yukarıdaki tool sonuçlarını kullanarak DOĞAL bir cevap ver.

MUTLAKA YAP:
- Task isimlerini ve priority'lerini göster (tool sonucunda varsa)
- Kullanıcıya follow-up soru sor (örn: "Hangi task'la başlamak istersin?", "Bugün kaç saatin var?")
- Kısa ve öz ol, max 3-4 cümle
- Türkçe cevap ver

Tool sonuçlarını olduğu gibi kopyalama, YORUMLA ve ÖZ olarak aktar.` },
      ]
