...
export const useGamificationStore = create<GamificationState>()(
  persist(
      ...
      addXP: (baseAmount, _action) => {
        set((state) => {
          ...
          return {
            ...
            level: newLevel,
            // Preserve the earliest oldLevel across consecutive level-ups (e.g. a chain of
            // achievement XP awards in one tick) so the notification spans the full range
            // instead of only the last hop.
            pendingLevelUp: newLevel > oldLevel
              ? { oldLevel: state.pendingLevelUp?.oldLevel ?? oldLevel, newLevel }
      ...
      checkStreak: () => {
        ...
        set((s) => {
          ...
          if (s.lastActiveDate === yesterdayStr) {
            ...
            // Streak broken (missed a day) or first-ever activity
            newStreak = 1
          }
    ...
    }),
    {
      ...
      version: 1,
      // v0 -> v1: the level table shrank from 101 levels to 10. A persisted `level` from
      // the old table (e.g. 45) no longer has a matching entry in LEVELS, which would make
      // getLevel()/getLevelTitle() fall back to LEVELS[0] ("Novice") and show a 100% XP bar.
      // `totalXP` is the only carried-over value we still trust, so recompute `level` and
      // `currentXP` from it against the new table; everything else passes through untouched.
